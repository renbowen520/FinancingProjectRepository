...
<div class="adminLeft">
	...
	<table class="tzlist" width="100%" border="1" bordercolor="#e9e9e9" cellspacing="0" cellpadding="0">
		...
		<c:forEach items="${touzi_list }"  var="s"  varStatus="var">
		  <tr>
		 <td>${s.serial_number }</td>
		  ...
		  <td>${s.interest }</td>
		  <td>
		  <c:if test="${s.ispayment  ==0 }">
		     已还款
		  </c:if>
		   <c:if test="${s.ispayment  ==1 }">
		    没有还款
		  </c:if>
		  </td>
		  <td>${s.create_date }</td>
	...
	<div class="listCount">总计<font color="#ff503f">${fn:length(touzi_list)} </font>笔</div>
</div>
